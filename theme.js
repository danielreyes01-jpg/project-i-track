...
function initializeStudentModularTracker() {
	...
	fetch('/api/auth/me', { credentials:'include' }).then((response) => response.ok ? response.json() : null).then((payload) => {
		...
		const style = document.createElement('style');
		style.textContent = '.student-modular-tracker{margin:0 0 16px;padding:18px;border:1px solid #cfe1e5;border-radius:17px;background:linear-gradient(145deg,#fff,#f0f8fb);box-shadow:0 9px 25px rgba(16,47,73,.06)}.student-tracker-head{display:flex;justify-content:space-between;align-items:flex-start;gap:14px;margin-bottom:14px}.student-tracker-head h2{margin:0 0 4px;color:#123b58}.student-tracker-head p{margin:0;color:#687c82;font-size:.82rem}.student-term-filter{display:flex;gap:6px;flex-wrap:wrap}.student-term-filter button{min-height:34px;padding:6px 11px;border:1px solid #bad4df;border-radius:999px;background:#fff;color:#275b77;font:700 .72rem Manrope,sans-serif;cursor:pointer}.student-term-filter button.active{background:#176f9d;border-color:#176f9d;color:#fff}.student-progress-layout{display:grid;grid-template-columns:repeat(4,minmax(0,1fr)) minmax(190px,1.25fr);gap:9px}.student-progress-stat,.student-progress-summary{padding:12px;border:1px solid #dce8eb;border-radius:12px;background:#fff}.student-progress-stat span{display:block;color:#687c82;font-size:.66rem;text-transform:uppercase}.student-progress-stat strong{display:block;margin-top:4px;color:#113d5a;font-size:1.35rem}.student-progress-summary{display:grid;align-content:center;gap:7px}.student-progress-summary strong{color:#147556;font-size:1rem}.student-progress-bar{height:9px;border-radius:999px;background:#e3edeb;overflow:hidden}.student-progress-bar i{display:block;width:0;height:100%;background:#268d6d;transition:width .35s ease}.student-next-modules{display:grid;gap:7px;margin-top:13px}.student-next-module{display:flex;justify-content:space-between;gap:10px;padding:9px 11px;border:1px solid #e0eaec;border-radius:10px;background:#fff;font-size:.74rem}.student-next-module small{display:block;color:#71817c}.student-next-module a{align-self:center;color:#126e91;font-weight:700;text-decoration:none;white-space:nowrap}@media(max-width:900px){.student-progress-layout{grid-template-columns:repeat(2,1fr)}.student-progress-summary{grid-column:1/-1}}@media(max-width:620px){.student-tracker-head{flex-direction:column}.student-progress-layout{grid-template-columns:1fr 1fr}.student-next-module{align-items:flex-start;flex-direction:column}}';
		document.head.appendChild(style);
		const tracker = document.createElement('section');
		tracker.id = 'studentModularTracker';
		tracker.className = 'student-modular-tracker';
		tracker.innerHTML = '<div class="student-tracker-head"><div><h2>📘 My Modular Progress</h2><p>Track your assigned modules and continue the activities that need attention.</p></div><div id="studentTermFilter" class="student-term-filter"><button class="active" type="button" data-term="all">All Terms</button><button type="button" data-term="1">Term 1</button><button type="button" data-term="2">Term 2</button><button type="button" data-term="3">Term 3</button></div></div><div class="student-progress-layout"><article class="student-progress-stat"><span>Total Modules</span><strong id="studentTrackerTotal">0</strong></article><article class="student-progress-stat"><span>Assigned</span><strong id="studentTrackerAssigned">0</strong></article><article class="student-progress-stat"><span>Ongoing</span><strong id="studentTrackerOngoing">0</strong></article><article class="student-progress-stat"><span>Completed</span><strong id="studentTrackerCompleted">0</strong></article><article class="student-progress-summary"><strong id="studentTrackerRate">0% complete</strong><div class="student-progress-bar"><i id="studentTrackerBar"></i></div><span id="studentTrackerMessage">Your modular activities will appear here.</span></article></div><div id="studentNextModules" class="student-next-modules"></div>';
		...
		const render = () => {
			...
			byId('studentTrackerRate').textContent = rate.toFixed(1) + '% complete';
			byId('studentTrackerBar').style.width = rate + '%';
			...
			byId('studentNextModules').innerHTML = next.length ? next.map((item) => '<div class="student-next-module"><span><strong>Term ' + Number(item.term || 1) + ' · ' + escapeHtml(item.resource_type || 'Module') + ' ' + Number(item.module_number || 1) + '</strong><small>' + escapeHtml(item.title || 'Learning activity') + ' · ' + (item.status === 'ongoing' ? 'Ongoing' : 'Ready to answer') + '</small></span><a href="learning-resources.html">' + (item.status === 'ongoing' ? 'Continue →' : 'Answer now →') + '</a></div>').join('') : '';
		};
