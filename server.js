...
function sanitizeUser(user) {
  return {
	...
	profile_image: user.profile_image_data ? `/api/profile-images/${encodeURIComponent(user.id)}?v=${encodeURIComponent(user.updated_at || "current")}` : (user.profile_image || ""),
...
app.post("/api/account/profile-image", requireLogin, profileImageUpload.single("profileImage"), async (req, res) => {
  try {
    ...
    const profileImage = `/api/profile-images/${encodeURIComponent(user.id)}?v=${encodeURIComponent(updatedAt)}`;
    await db("users").where({ id: user.id }).update({ profile_image: profileImage, profile_image_data: req.file.buffer, profile_image_mime_type: req.file.mimetype, updated_at: updatedAt });
